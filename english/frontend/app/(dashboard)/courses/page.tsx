"use client";
import { useState, useEffect, useCallback } from "react";
import { motion, AnimatePresence } from "framer-motion";
import { useRouter } from "next/navigation";
import {
  GraduationCap, Search, Filter, BookOpen, Clock, ChevronLeft,
  ChevronRight, X, Layers, Star, TrendingUp
} from "lucide-react";
import { socialApi } from "@/api/client";
import { cn } from "@/lib/utils";
import toast from "react-hot-toast";

const LEVELS = [
  { value: "", label: "Все уровни" },
  { value: "beginner", label: "A1 — Начинающий" },
  { value: "elementary", label: "A2 — Элементарный" },
  { value: "intermediate", label: "B1 — Средний" },
  { value: "upper-intermediate", label: "B2 — Выше среднего" },
  { value: "advanced", label: "C1 — Продвинутый" },
  { value: "proficient", label: "C2 — Свободный" },
];

const LEVEL_COLORS: Record<string, string> = {
  beginner: "text-green-400 bg-green-400/10",
  elementary: "text-teal-400 bg-teal-400/10",
  intermediate: "text-blue-400 bg-blue-400/10",
  "upper-intermediate": "text-violet-400 bg-violet-400/10",
  advanced: "text-orange-400 bg-orange-400/10",
  proficient: "text-red-400 bg-red-400/10",
};

const LEVEL_LABELS: Record<string, string> = {
  beginner: "A1",
  elementary: "A2",
  intermediate: "B1",
  "upper-intermediate": "B2",
  advanced: "C1",
  proficient: "C2",
};

function PatternBg({ id, className }: { id: string; className?: string }) {
  const patterns = [
    `url("data:image/svg+xml,%3Csvg width='20' height='20' viewBox='0 0 20 20' xmlns='http://www.w3.org/2000/svg'%3E%3Ccircle cx='2' cy='2' r='1.5' fill='white' fill-opacity='0.08'/%3E%3C/svg%3E")`,
    `url("data:image/svg+xml,%3Csvg width='20' height='20' viewBox='0 0 20 20' xmlns='http://www.w3.org/2000/svg'%3E%3Crect x='0' y='0' width='20' height='1' fill='white' fill-opacity='0.06'/%3E%3Crect x='0' y='0' width='1' height='20' fill='white' fill-opacity='0.06'/%3E%3C/svg%3E")`,
    `url("data:image/svg+xml,%3Csvg width='28' height='28' viewBox='0 0 28 28' xmlns='http://www.w3.org/2000/svg'%3E%3Cpath d='M0 28L28 0' stroke='white' stroke-opacity='0.06' stroke-width='1'/%3E%3C/svg%3E")`,
  ];
  const idx = id.charCodeAt(0) % patterns.length;
  return (
    <div
      className={cn("absolute inset-0", className)}
      style={{ backgroundImage: patterns[idx] }}
    />
  );
}

function CourseCard({ course }: { course: any }) {
  const router = useRouter();
  const levelLabel = LEVEL_LABELS[course.level] || "A1";
  const levelColor = LEVEL_COLORS[course.level] || LEVEL_COLORS.beginner;

  const gradients = [
    "from-blue-600 to-indigo-700",
    "from-violet-600 to-purple-700",
    "from-teal-600 to-emerald-700",
    "from-orange-600 to-amber-700",
    "from-rose-600 to-pink-700",
    "from-cyan-600 to-sky-700",
  ];
  const grad = gradients[course.id.charCodeAt(0) % gradients.length];

  return (
    <motion.div
      initial={{ opacity: 0, y: 12 }}
      animate={{ opacity: 1, y: 0 }}
      whileHover={{ y: -2 }}
      onClick={() => router.push(`/groups/${course.groupId}/courses/${course.id}`)}
      className="glass-card rounded-2xl overflow-hidden cursor-pointer group"
    >
      {/* Header */}
      <div className={`relative h-28 bg-gradient-to-br ${grad} overflow-hidden`}>
        <PatternBg id={course.id} />
        <div className="absolute inset-0 flex items-center justify-center">
          <span className="text-4xl drop-shadow">{course.emoji || "📖"}</span>
        </div>
        <div className="absolute top-2.5 right-2.5">
          <span className={cn("text-[11px] font-bold px-2 py-0.5 rounded-lg", levelColor)}>
            {levelLabel}
          </span>
        </div>
      </div>

      {/* Body */}
      <div className="p-4">
        <div className="text-xs text-muted-foreground mb-1.5 flex items-center gap-1">
          <span>{course.groupEmoji}</span>
          <span className="truncate">{course.groupName}</span>
        </div>
        <h3 className="font-semibold text-sm leading-snug mb-2 line-clamp-2 group-hover:text-primary transition-colors">
          {course.title}
        </h3>
        {course.description && (
          <p className="text-xs text-muted-foreground line-clamp-2 mb-3">{course.description}</p>
        )}
        <div className="flex items-center gap-3 text-xs text-muted-foreground pt-2 border-t border-border/30">
          <span className="flex items-center gap-1">
            <Layers size={11} />
            {course.blockCount || 0} блоков
          </span>
          {course.estimatedMinutes > 0 && (
            <span className="flex items-center gap-1">
              <Clock size={11} />
              {course.estimatedMinutes} мин
            </span>
          )}
        </div>
      </div>
    </motion.div>
  );
}

export default function PublicCoursesPage() {
  const [courses, setCourses] = useState<any[]>([]);
  const [loading, setLoading] = useState(true);
  const [search, setSearch] = useState("");
  const [level, setLevel] = useState("");
  const [page, setPage] = useState(1);
  const [totalPages, setTotalPages] = useState(1);
  const [total, setTotal] = useState(0);
  const [showFilters, setShowFilters] = useState(false);

  const fetchCourses = useCallback(async () => {
    setLoading(true);
    try {
      const data = await socialApi.getPublicCourses({ search, level, page, limit: 12 });
      setCourses(data.courses || []);
      setTotalPages(data.totalPages || 1);
      setTotal(data.total || 0);
    } catch {
      toast.error("Не удалось загрузить курсы");
    } finally {
      setLoading(false);
    }
  }, [search, level, page]);

  useEffect(() => { fetchCourses(); }, [fetchCourses]);

  // Reset page on filter change
  useEffect(() => { setPage(1); }, [search, level]);

  return (
    <div className="max-w-6xl mx-auto space-y-6 page-enter">
      {/* Header */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold flex items-center gap-2">
            <GraduationCap size={24} className="text-primary" />
            Публичные курсы
          </h1>
          <p className="text-sm text-muted-foreground mt-1">
            {total > 0 ? `${total} курсов доступно` : "Изучайте материалы сообщества"}
          </p>
        </div>
        <button
          onClick={() => setShowFilters(v => !v)}
          className={cn(
            "flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-medium transition-colors border",
            showFilters
              ? "bg-primary/10 border-primary/30 text-primary"
              : "bg-accent/50 border-border/40 text-muted-foreground hover:text-foreground"
          )}
        >
          <Filter size={14} />
          Фильтры
          {(level) && (
            <span className="w-2 h-2 rounded-full bg-primary" />
          )}
        </button>
      </div>

      {/* Search + filters */}
      <div className="space-y-3">
        <div className="relative">
          <Search size={16} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-muted-foreground" />
          <input
            type="text"
            placeholder="Поиск курсов..."
            value={search}
            onChange={e => setSearch(e.target.value)}
            className="w-full h-11 pl-10 pr-4 rounded-xl border border-input bg-background text-sm focus:outline-none focus:ring-1 focus:ring-ring"
          />
          {search && (
            <button onClick={() => setSearch("")} className="absolute right-3 top-1/2 -translate-y-1/2 text-muted-foreground hover:text-foreground">
              <X size={14} />
            </button>
          )}
        </div>

        <AnimatePresence>
          {showFilters && (
            <motion.div
              initial={{ opacity: 0, height: 0 }}
              animate={{ opacity: 1, height: "auto" }}
              exit={{ opacity: 0, height: 0 }}
              className="overflow-hidden"
            >
              <div className="flex flex-wrap gap-2 py-1">
                {LEVELS.map(l => (
                  <button
                    key={l.value}
                    onClick={() => setLevel(l.value === level ? "" : l.value)}
                    className={cn(
                      "px-3 py-1.5 rounded-xl text-xs font-medium transition-colors border",
                      level === l.value
                        ? "bg-primary text-primary-foreground border-primary"
                        : "bg-accent/50 border-border/40 text-muted-foreground hover:text-foreground"
                    )}
                  >
                    {l.label}
                  </button>
                ))}
              </div>
            </motion.div>
          )}
        </AnimatePresence>
      </div>

      {/* Grid */}
      {loading ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
          {Array.from({ length: 8 }).map((_, i) => (
            <div key={i} className="glass-card rounded-2xl overflow-hidden animate-pulse">
              <div className="h-28 bg-muted/30" />
              <div className="p-4 space-y-2">
                <div className="h-3 bg-muted/30 rounded w-3/4" />
                <div className="h-4 bg-muted/30 rounded" />
                <div className="h-3 bg-muted/30 rounded w-2/3" />
              </div>
            </div>
          ))}
        </div>
      ) : courses.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-20 text-center">
          <div className="w-16 h-16 rounded-2xl bg-accent/50 flex items-center justify-center mb-4">
            <BookOpen size={28} className="text-muted-foreground" />
          </div>
          <h3 className="font-semibold mb-1">Курсы не найдены</h3>
          <p className="text-sm text-muted-foreground max-w-xs">
            {search || level
              ? "Попробуйте изменить фильтры"
              : "Публичные курсы появятся здесь, когда администраторы опубликуют их"}
          </p>
          {(search || level) && (
            <button
              onClick={() => { setSearch(""); setLevel(""); }}
              className="mt-3 text-sm text-primary hover:underline"
            >
              Сбросить фильтры
            </button>
          )}
        </div>
      ) : (
        <>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
            {courses.map(course => (
              <CourseCard key={course.id} course={course} />
            ))}
          </div>

          {/* Pagination */}
          {totalPages > 1 && (
            <div className="flex items-center justify-center gap-2 pt-2">
              <button
                onClick={() => setPage(p => Math.max(1, p - 1))}
                disabled={page === 1}
                className="p-2 rounded-xl border border-border/40 text-muted-foreground hover:text-foreground hover:bg-accent/50 disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
              >
                <ChevronLeft size={16} />
              </button>
              <div className="flex items-center gap-1">
                {Array.from({ length: Math.min(5, totalPages) }).map((_, i) => {
                  let p = i + 1;
                  if (totalPages > 5) {
                    if (page <= 3) p = i + 1;
                    else if (page >= totalPages - 2) p = totalPages - 4 + i;
                    else p = page - 2 + i;
                  }
                  return (
                    <button
                      key={p}
                      onClick={() => setPage(p)}
                      className={cn(
                        "w-8 h-8 rounded-lg text-sm font-medium transition-colors",
                        page === p
                          ? "bg-primary text-primary-foreground"
                          : "text-muted-foreground hover:text-foreground hover:bg-accent/50"
                      )}
                    >
                      {p}
                    </button>
                  );
                })}
              </div>
              <button
                onClick={() => setPage(p => Math.min(totalPages, p + 1))}
                disabled={page === totalPages}
                className="p-2 rounded-xl border border-border/40 text-muted-foreground hover:text-foreground hover:bg-accent/50 disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
              >
                <ChevronRight size={16} />
              </button>
            </div>
          )}
        </>
      )}
    </div>
  );
}
